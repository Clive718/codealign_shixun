...
CONFIG="${CONFIG:-sft/configs/qwen3_code_lora_predict_rank16.yaml}"
GPU_ID="${GPU_ID:-0}"
...
if [ -n "${MODEL_PATH}" ]; then
  ...
  TMP_CONFIG="$(mktemp "${PROJECT_ROOT}/sft/outputs/qwen3_code_lora_predict_epo500/.predict_config_lora.XXXXXX.yaml")"
