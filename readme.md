...
github仓库：https://github.com/Clive718/codealign_shixun
---
...
sft 
├── configs 
│ ├── qwen3_code_full_predict.yaml 
│ ├── qwen3_code_full_sft.yaml 
│ ├── qwen3_code_lora_predict_rank16.yaml 
│ ├── qwen3_code_lora_predict.yaml 
│ ├── qwen3_code_lora_rank16.yaml
│ ├── qwen3_code_lora.yaml 
│ ├── qwen3_code_qlora_predict.yaml
│ ├── qwen3_code_qlora.yaml
│ ├── qwen3_code_repair_lora.yaml 
│ ├── qwen3_code_repair_predict.yaml 
│ ├── qwen15_code_full_predict.yaml 
│ ├── qwen15_code_full_sft.yaml 
│ ├── qwen15_code_lora_predict.yaml 
│ ├── qwen15_code_lora.yaml 
│ ├── qwen15_code_qlora_predict.yaml
│ └──qwen15_code_qlora.yaml
├── outputs
├── scripts 
│ ├── repair
│ │   ├── build_code_repair_dataset.py 
│ │   ├── error_analysis.py 
│ │   ├── evaluate_repair.py
│ │   ├── evaluate_repair.sh
...
│ ├── predict_lora.sh 
│ ├── predict_qlora.sh 
│ ├── prepare_code_sft_data.sh 
│ ├── prepare_code_lora_data.sh 
│ ├── prepare_data.sh 
│ ├── prepare_mbpp_sanitized_data.py
│ ├── run_all.sh 
│ ├── run_all_lora.sh 
│ ├── run_all_qlora.sh 
│ ├── train.sh 
...
本模块作为团队完整系统中的 代码生成与代码修复子模块，负责接收代码任务描述，生成对应代码，并对生成失败的代码进行自动修复。
通过对从A1模块经过处理的数据进行训练，预测，评测，修复等步骤，为后续的dpo模块提供可靠的训练数据。

（1）团队系统调用位置

团队系统首先调用基础代码进行训练并生成模型完成代码生成任务，当生成代码未通过测试用例时，可以调用错误分析与修复代码模块对错误代码进行修复。
...
包含instruction、input、output的一个python代码生成任务及其对应的输入和正确输出

典型输入格式如下：

{
    "instruction": "Fix the Python code according to the problem description and execution feedback.",
    "input": "\nProblem Description:\nWrite a python function that takes in a non-negative number and returns the number of prime numbers less than the given non-negative number.\n\nWrong Code:\ndef count_Primes_nums(n):\n    if n < 2:\n        return 0\n    count = 1\n    for i in range(2, n):\n        is_prime = True\n        for j in range(2, int(i**0.5) + 1):\n            if i % j == 0:\n                is_prime = False\n                break\n        if is_prime:\n            count += 1\n    return count\n\nExecution Feedback:\nTraceback (most recent call last):\n  File \"/tmp/mbpp_eval_u5irgxag/candidate_test.py\", line 18, in <module>\n    assert count_Primes_nums(5) == 2\n           ^^^^^^^^^^^^^^^^^^^^^^^^^\nAssertionError\n\n",
...
|返回内容|数据格式|说明|
|---|---|---|
|训练模型|jsonl/json|用于保存训练后的预测代码|
|修复后的代码|Python代码文本|用于替换原始错误代码|
|修复是否成功|Boolean|是否通过测试|
|修复分析报告|TXT / JSON|保存修复过程与统计信息|

示例输出：
{"line_no": 1, "task_id": 11, "prompt": "Write a python function to remove first and last occurrence of a given character from the string.", "predict": "<think>\n\n</think>\n\ndef remove_Occ(str1,char):\n  return str1.replace(char, '', 2)", "reference_code": "def remove_Occ(s,ch): \n    for i in range(len(s)): \n        if (s[i] == ch): \n            s = s[0 : i] + s[i + 1:] \n            break\n    for i in range(len(s) - 1,-1,-1):  \n        if (s[i] == ch): \n            s = s[0 : i] + s[i + 1:] \n            break\n    return s", "code": "def remove_Occ(str1,char):\n  return str1.replace(char, '', 2)", "syntax_ok": true, "passed": true, "passed_tests": 3, "total_tests": 3, "test_results": [{"passed": true, "error_type": "", "stdout": "", "stderr": ""}, {"passed": true, "error_type": "", "stdout": "", "stderr": ""}, {"passed": true, "error_type": "", "stdout": "", "stderr": ""}], "exact_match": false, "token_f1": 0.2}
...
类型	文件路径	用途
基础模型	./Qwen3-1.7B	代码生成基础模型
LoRA权重	./sft/outputs/qwen3_code_lora	代码生成模型
Repair LoRA权重	./sft/outputs/qwen3_code_repair_lora	代码修复模型
数据集配置	./sft/data/dataset_info.json	数据集注册
MBPP数据集	./mbpp	自动评测
修复样本	./sft/data/code_repair_train.json	Repair SFT训练
（5）联调过程中遇到的接口问题及解决方法
问题1：函数签名不一致

模型生成代码中的函数参数数量与测试用例要求不一致，例如：
...
在错误分析模块中新增 Function Signature Error 分类；
在 Repair Dataset 中加入执行反馈信息；
使用 Code Repair SFT 学习函数接口修复。
问题2：模型输出包含 <think> 标签
...
在 repair_execute.py 中增加正则表达式过滤：
...
设计 prepare_code_repair_data.py 自动完成数据格式转换，并统一转换为 Alpaca 格式。
...
| 问题 | 当前原因 | 后续改进 |
|---|---|---|
| 可视化界面不完善 | 训练消耗时间过多，没有充足制作可视化界面的时间 | 将训练进度，评测结果，错误判断，代码修复几个模块的内容放到可视化界面中|
| 训练耗时过多，用户无法及时查看训练进度，也无法及时查看修复结果。 | 当使用参数过多的模型时，训练时长会成倍增加 | 调整训练轮次，减少训练时间 |
| 修复错误代码后，用户无法及时查看修复结果 | 修复后的代码需要重新执行测试 | 自动重新验证修复结果 |
