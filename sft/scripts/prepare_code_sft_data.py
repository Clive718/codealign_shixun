...
def is_python_parsable(code: str) -> bool:
    ...
    使用 ast.parse 尝试解析。能解析成功说明代码语法完整、无 Python2
    残留（如 print 语句）、无被截断的代码块，这类样本对训练更有价值。
    """
    stripped = code.strip()
    if not stripped:
        return False
...
def is_task_complete(row: dict[str, str]) -> bool:
    ...
    完整性要求：instruction 非空、output 非空，且 output 不包含明显的
...
def high_quality_filter(
    ...
    返回 (合格样本列表, 被过滤样本列表)。合格样本会附带 task_type 字段，
    供后续均衡采样使用。
    """
    passed: list[dict[str, str]] = []
    bad_cases: list[dict[str, Any]] = []
